...
class ModelCrudTests {
	...
	void updateUser() {
		...
		User user2 = new User("name"+r.nextInt(RANGE), "second"+r.nextInt(RANGE));
	...
	void loadUser() {
		...
		UserDetails user2 = userDetailsService.loadUserByUsername(user.getUsername());
		assertEquals("usernames are not equal.", user.getUsername(), user2.getUsername());
